...
比赛已正式结束。为便于组织方对最终结果进行复现与核验，**微调赛道和不微调赛道排名前六的队伍均需提交代码及相关运行说明**。

请相关队伍及时加入本页面下方的微信交流群，方便后续进行代码提交、结果复现及技术报告撰写等事项的沟通。组织方将根据复现核验情况完成最终排名确认。
...
| 关系名称 | 英文（缩写） | 关系类型示例 | 说明 | 示例 |
|----------|-------------|----------|------|------|
| 包含 | CONTAINS (CON) | (CROP, VAR) | 用于明确的归属、从属、对应、别名、简称/全称、学名/俗名、同位说明关系 | 晋谷21号属于谷子 |
| 采用 | USES (USE) | (VAR, BM) | 用于明确的采用、使用、借助、通过某方法、技术、材料进行分析、筛选、育种、检测、定位等| 晋谷21号采用杂交育种 |
| 具有 | HAS (HAS) | (VAR, TRT) | 表达某对象“具有、表现出、具备、拥有”某性状时使用 | 晋谷21号具有抗倒伏 |
| 影响 | AFFECTS (AFF) | (ABS/GENE/MRK/QTL, TRT) | 用于明确的影响、调控、促进、抑制、响应、敏感、耐受、抗性等方向性作用关系 | 干旱胁迫影响产量 |
| 发生于 | OCCURS_IN (OCI) | (TRT/ABS/BIS, GST) | 用于发生于、测定于、调查于、处理于某生育时期或阶段 | 千粒重测定于成熟期 |
| 定位于 | LOCATED_IN (LOI) | (MRK/QTL/GENE, CHR) | 用于定位、映射、连锁、位于、关联、候选对应等分子证据关系 | KASP_Chr2_15Mb定位于qYLD2.2 |

注意：
 - 六类关系是粗粒度的，表格中的十三种关系类型只是示例，实际上的关系pair并不局限于这十三种。
...
- Track-A 赛道允许使用外部的数据，但最后必须确切的提供数据源以及对应外部数据；
- Track-B 赛道限制模型参数不超过7B，按通常认知的7B级别计算，而非绝对数值严格限制，因此选手可选择主流的7B级开源模型进行微调；
- Track-B 赛道要求在开发阶段采用的策略不会改变最终提交模型的单模型属性和参数规模。
...
- Track-B 基于 LLaMA Factory 框架，采用 LoRA 方式进行监督微调（SFT），对 Qwen2.5-7B-Instruct 模型进行训练，并完成后续推理与评估。
...
- **2026年6月18日**：测试A榜 Leaderboard 发布  
- **2026年6月24日**：测试B榜（测试集）数据发布、Leaderboard 发布  
- **2026年7月3日**：评测任务结束  
- **2026年7月3日–9日**：任务组织者、获奖队伍撰写技术报告  
- **2026年7月10日**：提交技术报告  
...
参赛队伍需撰写技术报告，报告可使用中文或英文撰写。报告统一使用 **CCL 2024 论文模板**，模板下载链接为：[http://cips-cl.org/static/CCL2024/downloads/ccl2023_template.zip](http://cips-cl.org/static/CCL2024/downloads/ccl2023_template.zip)。

报告正文不得超过 **6 页**，参考文献页数不限。报告应至少包含 **模型介绍、评测结果、结果分析与讨论、参考文献** 四个部分。会议投稿需统一使用 **LaTeX 模板**。

由于本次会议采用双盲审稿，作者姓名和单位不能出现在投稿论文中。因此，作者在引用本人已有工作时，不应采用“我们提出……”等可能暴露身份的表述，建议使用第三人称方式，例如“作者姓名等提出……”。

不符合上述格式要求的论文，将不经过完整审稿流程，直接被拒稿。
